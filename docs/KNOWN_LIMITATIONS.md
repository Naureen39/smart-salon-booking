...
- **No mid-flow topic-switch memory.** Once a session's `intent` is set to `book_appointment`, every subsequent message is treated as continuing that flow, there's still no context stack, so a genuine mid-booking topic change back to booking-specific free text right after an aside can't be distinguished from the aside itself. A pure FAQ aside mid-booking ("actually, what are your hours?") is answered correctly, though: the same extraction call that handles booking free text can also classify a message as `faq` (per its own contract), and when it does, the orchestrator now routes to the real RAG-grounded FAQ answering path rather than trusting that call's own free-form guess at the answer, found in manual end-to-end testing to confidently state business hours that didn't match the actual seeded FAQ answer, a real hallucination handed straight to a user. Booking progress (the slots collected so far) survives the aside so the flow resumes afterward. **To upgrade further**: a real context stack in `app/ai/orchestrator.py::process_turn` for cases beyond a single-turn aside.
